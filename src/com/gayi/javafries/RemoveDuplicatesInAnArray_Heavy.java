...
public class RemoveDuplicatesInAnArray_Heavy {
		...
		public void avoidDuplicates(int[] arr) {
			...
			while (mp < arr.length - 1) {
				//// if arr[cp] equals to arr[mp]
				if (arr[cp] == arr[mp]) {

					// Iterating thru the array till arr[cp]==arr[mp]
					while (arr[mp] == arr[cp]) { // 1 1 2 2 3 4
					...
					System.out.println("incremented  cp :::" +cp + " mp ::" + mp);
